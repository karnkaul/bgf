...
class IDisplay : public IService {
  public:
	...
	[[nodiscard]] virtual auto get_ui_view() const -> RenderView const& = 0;

	virtual void set_world_space(glm::vec2 size) = 0;
	virtual void set_ui_space(glm::vec2 size) = 0;
	...
	[[nodiscard]] auto get_ui_space() const -> glm::vec2 { return get_ui_view().viewport; }
	...
	[[nodiscard]] auto project_to_ui(glm::vec2 fb_point) const -> glm::vec2 { return get_render_device().project_to(get_ui_view().viewport, fb_point); }
	[[nodiscard]] auto unproject_to_ui(glm::vec2 pointer) const -> glm::vec2 { return unproject(get_ui_view(), pointer); }
